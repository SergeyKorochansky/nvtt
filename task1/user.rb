...
class User
  extend Forwardable
  def_delegators :@db_data, :name, :digest, :salt, :unsuccessful_tries, :last_incorrect_login_try_at,
                 :name=, :digest=, :unsuccessful_tries=, :last_incorrect_login_try_at=
  ...
  def can_login?
    unsuccessful_tries <= 3 || ( last_incorrect_login_try_at + LOCK_SECONDS <= Time.now.to_i )
  end
